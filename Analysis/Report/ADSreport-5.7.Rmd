...
The main limitation comes from the process of text analysis. 
First, as our dataset is kind of large, in order to simplify programming process and get a quick overview, we creat our own word package in which we select and put the words manully. For example, *fruit*,*acid*,*palat*,*aroma*,*cherri*,*tannin*,*ripe* are the target words describing the feeling of testing wine. It's a manual process which shows limitation to get a thorough exploration of descriptions. We are only able to type in some typical words related to the testers' feeling about wine instead of put every single word into the words package. Besides, it's a manual process which means when there's any change in the dataset, this process may not work at all.
Second, when building the model, all the words are coming from previous dataset which means we do recommendation based on a limited words package. When there's a new word coming up in the descriptions, we cannot make accurate recommendatioin. 
