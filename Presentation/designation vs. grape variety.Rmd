...
percentage.table <- function(x, digits = 2){
  ...
  percentage.tab <- 100*x/(sum(grape.variety[,"grape.variety.number"]))
  rounded.tab <- round(x = percentage.tab, digits = digits)
  return(rounded.tab)
}
...
renderPlot({
  library(ggplot2)
  library(ggthemes)
  grape.variety<-dat[,.("grape.variety.number"=length(unique(get(variety.name)))),by=designation.name]
  
  setorderv(grape.variety,cols = "grape.variety.number",-1)
  grape.variety <-grape.variety[-1,]
  data1 <- grape.variety[1:input$designation.number,]
  setorderv(data1,cols = "grape.variety.number",-1)
  
  ggplot(data=data1,aes(designation,grape.variety.number,fill=designation))+
  geom_bar(stat = "identity",position = "dodge")+
  theme_wsj()+
  theme(axis.title = element_blank())+
  ggtitle("Number of Grape Variety ")
  
  #tab <- percentage.table(x = data1[1:input$designation.number,"grape.variety.number"])
  
  #if(input$show.percentage ==TRUE){
    #space_val = 0
    #text(x = -0.4 + 1:input$designation.numbe * (1+space_val), y = tab, labels = sprintf("%.1f%%", tab), pos = 3)
  #}
}
)
#percentage.table(x = data1[1:5,"grape.variety.number"])
#100*data1[1:5,"grape.variety.number"]/sum(grape.variety[,"grape.variety.number"])
```
