---
title: "Applied Data Science Final Project: Analysis on Wine Reviews"
output:
...
This report addresses the follwing : 
- What are the factors that influence prices?
- What kind of segmentation makes sense for price?
- Create a recommendation system for a user 
- Perform a text analysis on the user review to understand sentiment. 
...
<p> We first checked the percentage of missing values for each variable. **Country, province, region_1 and region_2** are varibales giving geographic information about the wine, since the majority of **region_2** and 16.4% of **region_1** are missing, we choose to focus on **country** unless more detailed geographic information are needed. We also assign the value of the highest level region to the lowest level for the missing components. </p>

We then examined the unique values for each variable. **Designation, winery, title** have many unique values, hence we decided to treat them as random effects but not predictors when modeling.<br>

**Price** of the wine ranges from 4 to 3300 with some extreme values. We then visualized **price** removed all the variable that was NA. 

Variable **points** ranges from 80 to 100. Its distribution was close to a bell shape. <br>

**Description**, **taster_name** and **taster_twitter_handle** provides information about the taster and their reviews on the wine. We examined these variables later based on the needs for text analysis and recommendation systems building. We also assumed that the missing values for taster name were submitted anonymously. Those values have been altered to reflect anonymous. 

**Designation** We researched wine designation to further understand Wine Designation and according to Napa Valley Vinters: Vineyard Designations Optional. Many wineries name the vineyard where the grapes were grown because the winery believes the property produces an unusually high-quality grape. The winery or an independent grower may own the vineyard. When using a vineyard designation on a wine label, federal regulations require that 95% of the grapes be grown in the named vineyard.



```{r percentage missing}
tab  <- dat[,lapply(X=.SD,FUN="miss.value")]

rownames(tab) <- 'Percentage of missing value'
datatable(tab)
...
paste0( dat.row - nrow(dat), ' rows were removed or ', round((dat.row / nrow(dat)- 1) * 100,2),'%' , 'of the data')
```
...
Suppose we are exploring sales status of different wines in our website. What do we care about the most? The answer might be the `r points.name` the testers give out. But some reality need to be considered. 
...
```{r}
# filter data set
subdat <- dat[, c('variety','country','points', 'province', 'winery', 'description')]
...
To give an accurate evaluation, we start from estimating sentiment behind description, which is sentiment analysis. There are a default classification criteria of "negative" and "positive", which helps us to roughly divide description into two categories. From the result, we get more positive descriptions than negative one which kind of makes sense because we just use data with points from 80 to 100. There, the proportion of positive description is 79% and that of negative description is roughly 21%.

The result kind of shows discrepancy. As we mention above, we shift data with points above 80, which is a relatively higher score with a scale from 0 to 100. We didn't expect a negative description that much. In order to have a more realistic understanding of the result, we create a new scale which is from 80 to 100 and we assume points above 90 is "positive" points while points below 90 is "negative" points. Thus, we make a prediction the distribution of new-scaled points is consistent with the description of negative and positive. However, from what we get we conclude that the prediction is not correct, which indicates that although the final point is positive, we still have negative description. Thus, it's necessary to analyze description with a more specific classification which is text analysis.


```{r sentiment analysis}

# subdat includes all values with description
subdat %>%
...
There are two main factors which influence the quality of a type of wine, one is a natural cause and one is a man-made cause. The country of origin is an important component in natural cause. Thus, we want to specify if wines from different countries will have different descriptions from testers. With the sentiment perspective, it will be shown as the difference in the distribution of negative and positive sentiment in the description.

Then, we draw a graph of sentiment distribution to see the difference among wines from different counties. As some countries’ wine does not have sufficient recorded review, we just show countries with a large amount of data. 

From the result, we can tell that there exists a difference among countries. Meanwhile, wines that from most of the famous places like, France, South Africa have a larger proportion of positive description which is consistent with our intuition.
...
Dividing sentiment in "negative" and "positive" is somewhat sketchy and we have another way to make a more specific classification. We extract more emotions from descriptions such as "disgust", "joy" etc. To our surprise, there exist words containing sentiment like "anger", "sad" which seems a little bit strange in a wine description. This interesting finding gives us more hint on our further text analysis.
...
Before the analysis of text data, we did some preparation. 
  
- **Tokenization**: Cut the reviews into single words to calculate the frequency of their appearance.

- **Remove "stopwords"**: Remove "stopwords" such as "the", "a", "are", which are meaningless but necessary in the sentence. 

- **Stem the words**: Stem the words in order to reduce the number of total different words appear in the reviews. For example, this step change "fruit", "fruity", "fruits" all into "fruiti". Even though "fruiti" is not actually a word, it is still understandable.
...
For example, in the text 1, fruit has a lower score than acid. We believe that "fruit" has less information because most of the reviews include "fruit". On the contrary, acidity is more useful in this case, since only part of the wines is acid, which makes it more unique and characteristic, and more suitable as a variable.

## 2.4 Prediction

Based on the word cloud plot and TF-IDF, we selected the top 100 frequent words. Then we deleted some meaningless words for this project, such as "wine", "well", "show" and etc. In the end, we chose 58 words as our new variables. 
...
lm.model.summary <- linear.regression.summary(model)[,c(1:3,5:7)]

datatable(lm.model.summary)
```

F statistic is 483.7 and p-value is smaller than $2.2*10^{-16}$. Thus, the model is significant. Also, most variables are significant in this model. But adjusted R-squared is 0.2353, which is not really high. Considering that testers may not write down all aspect of a wine and the standard of evaluating a wine is rather complicated, we still think this is a nice try.
...
Second, we built a linear regression model. Among the words with high frequency, we manually selected some words as the predictors based on the domain knowledge. For example, *fruit*,*cherri*,*plum*,*oak*,*chocol*, are some common words used for describing the flavors of the wine. Therefore, the result is interpretable since each predictor is a meaningful word related to the wine. However, since we're focusing on high-frequency words, it's hard to get a thorough exploration of descriptions. Also, as this a manually process, any change in the dataset will impact the significance of these predictors.<br>

Lastly, when building the model, all the words are coming from the previous dataset which means we draw conclusions based on that. When there's a new word coming up in the descriptions, we won't be able to handle that. This is no longer the situation where a new observation brings new values for existing predictors, but the case when a new observation brings new predictors. <br>

## Areas of Future Investigation
To tackle the limitations of text analysis in this project, we tried another approach. We first followed the same procedure for text cleaning. After getting the word-document matrix, we used SVD (singular vector decomposition) to reduce the number of predictors. However, to use SVD, we need to fully trust the math since each column is no longer a word that we can interpret. There were signals missing in the dimension reduction process and we didn't know what those are. Then we built a decision tree model to predict points. However, our model ended up predicting the same point for all the wine, hence we ended up going back to the original method for now. Even so, this is something interesting for us to do a future investigation. A plausible step is to investigate on feature engineering. <br>


# 3. Recommendation System

```{r}
dat.recom <- dat[, mean(get(points.name)), by = .(get(taster.name), get(variety.name))]
setnames(dat.recom, old = c('get','get.1','V1'), new = c('Taster Name','Variety','Point'))
```

```{r real rating class}
# convert into a s4 class 
data_matrix <- as(dat.recom, 'realRatingMatrix')
...
The similarity matrix allows you to understand how similar users are to each other. We are going to show the top 4. 
